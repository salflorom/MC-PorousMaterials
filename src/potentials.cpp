...
Eigen::Vector2d MC::EAMNa_Pot(int ithBox, int ithSpecies, int jthSpecies, int index){
	Particle ithPart, jthPart;
	Eigen::Vector2d energy;
	double rij;
...
double MC::EAMNa_EmbPot(double rho){
	Tools tls;
	Eigen::Matrix<double,10,1> rhoIntervals{1.00000,  0.90000,  0.800000,  0.700000,  0.620000,  0.280000,  0.050000,  0.030000,  1.400000,  0.000000};
	Eigen::Matrix<double,10,1> aValues{0.00000, -0.33140, -0.329781, -0.328243, -0.331155, -0.331845, -0.216727,  0.194462,  0.253218, -0.305496};
	Eigen::Matrix<double,10,1> bValues{0.00000,  0.00000, -0.032380,  0.001620,  0.056620, -0.039380, -0.637780, -2.937780, -2.937780,  0.129520};
	Eigen::Matrix<double,10,1> cValues{0.00000,  0.16190, -0.170000, -0.275000,  0.600000,  0.880000,  5.000000,  0.000000,  0.000000, -0.150000};
...
double MC::EAMNa_eDens(double radius){
	Eigen::Vector3d pValues(0, 3.4418, 1.0245);
	return pValues(1)*exp(-pValues(2)*radius);
}
double MC::EAMNa_PairPot(double radius){
	Tools tls;
	double phi = 0;
	Eigen::Matrix<double,12,1> rIntervals{0.0, 2.55, 2.80, 2.95, 3.45, 3.95, 4.45, 4.95, 5.45, 5.95, 7.45, 10.78};
	Eigen::Matrix<double,12,7> bValues{
...
Eigen::Vector2d MC::EAMK_Pot(int ithBox, int ithSpecies, int jthSpecies, int index){
	Particle ithPart, jthPart;
	Eigen::Vector2d energy;
	double rij;
	double rho=0, phiLow=0;
...
double MC::EAMK_EmbPot(double rho){
	Tools tls;
	Eigen::Matrix<double,8,1> rhoIntervals{1.00000,  0.86000,  0.760000,  0.560000,  0.440000,  0.280000,  1.150000,  0.000000};
	Eigen::Matrix<double,8,1> aValues{0.00000, -0.24030, -0.237254, -0.241903, -0.250201, -0.238019, -0.206929, -0.236804};
	Eigen::Matrix<double,8,1> bValues{0.00000,  0.00000, -0.043512,  0.136488, -0.053512, -0.149512, -0.239112,  0.046620};
	Eigen::Matrix<double,8,1> cValues{0.00000,  0.15540, -0.900000,  0.475000,  0.400000,  0.280000, -1.200000,  0.072000};
	double phi = 0;
	int mValue = 2;
...
double MC::EAMK_eDens(double radius){
	Eigen::Vector3d pValues(0, 3.7461, 0.8400);
	return pValues(1)*exp(-pValues(2)*radius);
}
...
Eigen::Vector2d MC::EAMRb_Pot(int ithBox, int ithSpecies, int jthSpecies, int index){
	Particle ithPart, jthPart;
	Eigen::Vector2d energy;
	double rij;
	double rho=0, phiLow=0;
...
double MC::EAMRb_EmbPot(double rho){
	Tools tls;
	Eigen::Matrix<double,8,1> rhoIntervals{1.00000,  0.90000,  0.760000,  0.650000,  0.500000,  0.280000,  1.450000,  0.000000};
	Eigen::Matrix<double,8,1> aValues{0.00000, -0.39630, -0.395189, -0.394587, -0.390822, -0.367575, -0.192569, -0.373802};
	Eigen::Matrix<double,8,1> bValues{0.00000,  0.00000, -0.022220,  0.013620, -0.082080, -0.227880, -1.363080,  0.099990};
	Eigen::Matrix<double,8,1> cValues{0.00000,  0.11110, -0.128000,  0.435000,  0.486000,  2.580000,  1.200000,  0.045000};
...
double MC::EAMRb_eDens(double radius){
	Eigen::Vector3d pValues(0, 4.4417, 0.8192);
	return pValues(1)*exp(-pValues(2)*radius);
}
...
Eigen::Vector2d MC::EAMGa_Pot(int ithBox, int ithSpecies, int jthSpecies, int index){
	Particle ithPart, jthPart;
	Eigen::Vector2d energy;
	double rij;
	double rho=0, phiLow=0;
...
double MC::EAMGa_EmbPot(double rho){
	Eigen::Matrix<double,7,1> rhoIntervals{1.00000,  0.92000,  0.870000,  0.800000,  0.750000,  0.650000,  1.400000};
	Eigen::Matrix<double,7,1> aValues{0.00000, -1.91235, -1.904030, -1.897380, -1.883520, -1.852620, -1.822820};
	Eigen::Matrix<double,7,1> bValues{0.00000,  0.00000, -0.208000, -0.058000, -0.338000, -0.898000,  0.302000};
	Eigen::Matrix<double,7,1> cValues{0.00000,  1.30000, -1.500000,  2.000000,  5.600000, -6.000000,  2.000000};
	double phi = 0;
...
double MC::EAMGa_eDens(double radius){
	Eigen::Vector3d pValues(0, 2.24450, 1.2);
	return pValues(1)*exp(-pValues(2)*radius);
}
double MC::EAMGa_PairPot(double radius){
	Eigen::Matrix<double,7,1> rIntervals{0.0, 2.15, 2.75, 3.35, 4.00, 6.50, 8.30};
	Eigen::Matrix<double,9,6> aValues{
		{0.0, -0.65052509307861e-01, -0.15576396882534e+00, -0.13794735074043e+00, 0.13303710147738e-01,  0.00000000000000e+00},
